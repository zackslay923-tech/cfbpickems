...
import useScoreboard from "./lib/useScoreboard"; // SCOREBOARD hook
...
function LeaderboardPage({ user, isAdmin, setPage }) {  // DEV: CFBD diagnostics — verify token retrieval/log (no CFBD API calls)
  ...
  const [showScorebug, setShowScorebug] = useState(() => { try { const v = localStorage.getItem("showScorebug"); return v === "1"; } catch { return false; } });
...
// SCOREBOARD HOOK v3 (config-driven fixture)
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "config", "cfbd"), (s) => {
      try {
  ...
  const [sbHardStop, setSbHardStop] = useState(() => {
  try { const v = localStorage.getItem("sbHardStop"); return v === null ? true : v === "1"; } catch { return true; }
}); // ADMIN: Hard Stop (default ON; persisted)
...
const [sbLocalFixture, setSbLocalFixture] = useState(() => {
  try { const v = localStorage.getItem("sbLocalFixture"); return v === "1"; } catch { return false; }
    ...
    : (cfg?.testMode
        ...
        : (cfg?.mode === "on"
            ...
            : (hasToken && cfgEmpty
                ? "cfbd"
                : "none")));
const sbSource = sbHardStop ? "none" : sbSourceRaw;
    ...
    try {
      ...
      for (const g of (Array.isArray(arr) ? arr : [])) {
        ...
        results[id] = {
          ...
          status: status || (winner ? "final" : null),
...
'
    ...
    <button
      onClick={(e) => { e.preventDefault(); setSbHardStop(v => !v); }}
      style={{ padding:"4px 8px", borderRadius:6, border:"1px solid rgba(255,255,255,.2)", background: sbHardStop ? "#B91C1C" : "#065F46", color:"#fff", fontWeight:600 }}
      ...
      <td key={"sb-" + g.id} style={{ ...cell, textAlign: "center" }}>
        <Scorebug
            ...
            live={(() => {
              ...
              const key = norm(g?.away) + "__" + norm(g?.home);
              const liveItem = (sbMap && sbMap.get) ? sbMap.get(key) : null;
    ...
    try {
      ...
      all.forEach((d) => {
        ...
        merged[key] = {
          ...
          status: r.status ?? (winner ? "final" : null),
