...
const winnerCellStyleFn = (results, cell, g) => {
  const COL_W = (140); const base = { ...cell, fontWeight:700, fontSize:"15px", textAlign:"center", width: 140, minWidth: 140 };
...
function Header({ user, isAdmin, setPage }) {
  ...
  return (
    ...
    location.hash="#/picks"; location.hash="#/picks"; setPage("picks");}}>Picks</a>
        <a href="#" onClick={(e)=>{e.preventDefault(); location.hash="#/leader"; location.hash="#/leader"; setPage("leader");}}>Leaderboard</a>
        {isAdmin && <a href="#" onClick={(e)=>{e.preventDefault(); location.hash="#/admin"; location.hash="#/admin"; setPage("admin");}}>Admin</a>}
...
async function listGames({ year, week, includedOnly }) {
  ...
  const q = query(col, where("year","==",year), where("week","==",week));
  ...
  if (includedOnly) {
    for (let i = items.length - 1; i >= 0; i--) {
      const g = items[i];
      if (!(g?.included === true || g?.gameday === true)) items.splice(i, 1);
    }
  }
...
const normVenmo = (s) => String(s||"").trim().toLowerCase().replace(/^@+/, "");const confirmAndSubmit = async () => {
  if (!pending) return;
  setMsg("Saving...");
  try {
...
<div style={{ fontWeight:800, fontSize:  30, textDecoration:"underline" }}>Welcome to Week {week}!</div>
...
while (i < seq.length) {
  ...
  return <>
    {spans.map((sp, idx) => sp.type === "group" ? (
      ...
      <th key={"grp-gameday-"+idx}
        ...
        <button type="button" onClick={confirmAndSubmit}>Confirm & Submit</button>
      </div>
    </div>
  </div>
)}
  ...
  <ul style={{ paddingLeft: "1.25rem", margin: 0 }}>
    <li><strong>Weekly Picks:</strong> Each week you'll pick winners from a curated slate â€” marquee matchups, AP Top 25 games, all Florida FBS teams, plus a few randoms to keep it interesting.</li>
...
function LeaderboardPage({ user, isAdmin, setPage }) {
  ...
  const [lbPicksPublic, setLbPicksPublic] = useState(false);
  useEffect(() => {
...
try {
  ...
  if (lbLocked && Number(year) === Number(live?.year) && Number(week) === Number(live?.week)) {
      // Clear selected week if it has NO picks (safety guard)
  const clearWeekIfNoPicks = async () => {
    try {
      const Y = Number(year), W = Number(week);
...
<Field label="Previous weeks">
  ...
  const headerCell = { ...cell, textAlign:"center", width: 136, minWidth: 136, whiteSpace:"normal", overflowWrap:"anywhere", wordBreak:"break-word", fontWeight:700, fontSize:"15px", paddingTop: 12, paddingBottom: 12, lineHeight: 1.244 };
...
while (i < seq.length) {
  ...
  <div style={{ whiteSpace:"nowrap" }}>Tiebreaker</div>
              ...
              <tr>
                ...
                {displayGames.map(g => (
                  ...
                  <td key={g.id} data-game-id={g.id} style={{ ...winnerCellStyleFn(results, cell, g), width: 136, minWidth: 136, fontStyle:"italic", fontSize: fitFontByLen(String(results[g.id]?.winner||"").length) }}>{winnerCell(g)}</td>
                
                ))}{gameday ? (<td key="tb_win" style={{ ...cell, textAlign:"center", fontStyle:"italic", width: 136, minWidth: 136 }}></td>) : null}
              </tr>
            </thead>
            <tbody>
              {players.map(p => (
  ...
  return (
                      <td key={g.id} data-game-id={g.id} style={{ ...pickCellStyle(g.id, choice), width: 136, minWidth: 136 }}><div style={{display:"flex",justifyContent:"center"}}>{label}</div></td>
                    );
                  })}
                {gameday ? (
  <td key={"tb_"+(p.email||p.name||p.code||p.id)}
      style={{ ...cell, textAlign:"center", width: 136, minWidth: 136 }}>
...
useEffect(() => {
  ...
  const [appCfg, setAppCfg] = useState({ leaderboardLocked: false, leaderboardPicksPublic: false });
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "config", "app"), (s) => {
      const d = s.data() || {};
      setAppCfg({ leaderboardLocked: !!d.leaderboardLocked, leaderboardPicksPublic: !!d.leaderboardPicksPublic });
    });
    return () => unsub();
  }, []);
      ...
      <Card style={{ maxWidth: 1200 }}>
        ...
        <Row style={{ marginTop: 6, marginBottom: 6 }}>
          <button onClick={toggleLeaderboardLock}>
            {appCfg.leaderboardLocked ? "Turn ON Leaderboard" : "Turn OFF Leaderboard"}
          </button>
          <div style={{ color:"#9aa4c7", fontSize:13, marginLeft:8 }}>
            Status: {appCfg.leaderboardLocked ? "OFF (locked)" : "ON"}
          </div>
        </Row>
        <Row style={{ marginTop: 6, marginBottom: 6 }}>
